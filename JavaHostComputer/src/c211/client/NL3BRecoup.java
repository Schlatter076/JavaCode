...
import c211.db.NL3BRecoupData;
import c211.db.RecoupDataTool;

public class NL3BRecoup {
  ...
  public static void getRecoup() {
    EventQueue.invokeLater(new Runnable() {
      public void run() {
        try {
          NL3BRecoup window = new NL3BRecoup();
  ...
  public boolean isModify() {
    ...
    try {
      ...
      RecoupDataTool.updateNl3b_recoup(data);
      return true;
    } catch (Exception e1) {
      return false;
    }
  ...
  public Vector<Object> addRowDatas() {
    ...
    List<NL3BRecoupData> list = RecoupDataTool.getNl3bRecoupData();
    if(list != null) {
      for(Iterator<NL3BRecoupData> it = list.iterator(); it.hasNext(); ) {
        Vector<String> vt = new Vector<>();
        NL3BRecoupData cd = it.next();
        vt.add(" ");
